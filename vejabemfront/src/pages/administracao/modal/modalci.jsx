...
const ModalCI = (props) => {
    const { data, mutate} = useAxios(`/news/allci/`)
    ...
    const AtualizarImg = () => {
        if (tittle && file ) {
            ...
            api.post(`/news/allci/`, formData, { headers : { 'Content-Type' : 'multipart/form-data'}})
            .then((res) => mutate() )

        } else {
            alert('Não é possivel enviar uma CI sem conteúdo')
        }
    }

    const Activate = (id, bolean) => {
        api.patch(`/news/allci/${id}/`, { activo: bolean })
        .then((res) => mutate())
    }

    const DeletePost = (id) => {
        api.delete(`/news/allci/${id}/`)
        .then((res) => mutate())
    }
    return (
        <Modal
